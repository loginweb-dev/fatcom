...
function ohSnap(text, options) {
  ...
  var $container = $('#'+options['container-id']),
    ...
    if (options.color) {
      color_markup = 'oh-alert-' + options.color;
    }

    // Generate the HTML
    var html = $('<div class="oh-alert ' + color_markup + '">' + icon_markup + text + '</div>').fadeIn(options['fade-duration']);
...
function ohSnapX(element, options) {
    ...
    if (typeof element !== "undefined") {
        ...
        $('.oh-alert').fadeOut(options.duration, function() {
            $(this).remove();
        });
    }
}
