...
@section('page_title', 'Depositos')

@if(auth()->user()->hasPermission('browse_depositos'))
    @section('page_header')
        <h1 class="page-title">
            <i class="voyager-archive"></i> Depositos
        </h1>
        @if(auth()->user()->hasPermission('add_depositos'))
        <a href="{{route('depositos_create')}}" class="btn btn-success btn-add-new">
    ...
    @section('content')
                                    ...
                                    <div class="col-md-12">
                                        ...
                                        <form id="form-search" class="form-search">
                                            <div class="input-group col-md-4">
                                                <input type="text" id="search_value" class="form-control" name="s" value="{{$value}}" placeholder="Nombre o dirección">
                                ...
                                <div class="table-responsive">
                                    <table id="dataTable" class="table table-bordered table-hover">
                                        ...
                                        <tbody>
                                            @php
                                                $cont = 0;
                                                $estado_inventario = ['<label class="label label-danger">Cerrado</label>', '<label class="label label-success">Abierto</label>'];
                                            @endphp
                                            @forelse ($registros as $item)
                                                <tr>
                                                    ...
                                                    <td class="no-sort no-click text-right" id="bread-actions">
                                                        ...
                                                        <a href="{{route('depositos_edit', ['id'=>$item->id])}}" title="Editar" class="btn btn-sm btn-primary edit">
        ...
        {{-- modal delete --}}
        <form action="{{route('depositos_delete')}}" method="POST">
        ...
        <script>
            $(document).ready(function() {
                ...
                $('#form-search').on('submit', function(e){
                    ...
                    window.location = '{{url("admin/depositos/buscar")}}/'+value;
                });
            });
        </script>
    @stop
