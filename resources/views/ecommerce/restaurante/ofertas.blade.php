...
@section('meta-datos')
    ...
    <meta property="og:url"           content="{{route('ofertas_ecommerce')}}" />
    <meta property="og:type"          content="E-Commerce" />
    <meta property="og:title"         content="Productos en ofertas - {{setting('empresa.title')}}" />
    <meta property="og:description"   content="Echa un vistazo a nuestros productos que tenemos en oferta para ti." />
    <meta property="og:image"         content="{{ url('storage').'/'.str_replace('\\', '/', setting('admin.imagen_ofertas')) }}" />
@endsection
...
@section('content')
    <main id="contenido" class="col-md-9 col-sm-12">
        <h1 class="display-5">Productos en oferta <div class="fb-like" data-href="{{route('ofertas_ecommerce')}}" data-width="" data-layout="button_count" data-action="like" data-size="large" data-show-faces="true" data-share="true"></div></h1>
        @php
            $cont = 0;
        @endphp
        @forelse ($productos as $item)
            @php
                ...
                if($item->tipo_descuento=='porcentaje'){
                    $precio_actual -= ($precio_actual*($item->monto_descuento/100));
                }else{
                    $precio_actual -= $item->monto_descuento;
                }
            @endphp
