                ...
                <table class="table table-hover shopping-cart-wrap">
                    ...
                    <tbody>
                        ...
                        <tr>
                                ...
                                <figure class="media">
                                    ...
                                    <figcaption class="media-body">
                                        ...
                                        <dl class="dlist-align small">
                                            <dt>Precio</dt>
                                            <dd>
                                                <var class="price text-info" id="label-precio{{$item->id}}">{{$item->moneda}} {{$precio_actual}}</var>
                                                <input type="hidden" class="form-control input-precio" data-id="{{$item->id}}" name="precio[]" value="{{$precio_actual}}">
                                            </dd>
                                            @php
                                                $costo_envio = '';
                                            ...
                                            @endphp
                                            <dt>Cantidad</dt>
                                            <dd><input type="number" @if($costo_envio === '') disabled @endif style="width:75px;padding:0px 10px" class="form-control" onchange="calcular_total({{$item->id}})" onkeyup="calcular_total({{$item->id}})" name="cantidad[]" id="input-cantidad{{$item->id}}" value="1" min="1" step="1"></dd>
                            ...
                            <td>
                                @switch(setting('admin.modo_sistema'))
                                    ...
                                    @case('restaurante')
                                        <dl>
                                            <dt>Descripción</dt>
                                            <dd><p>{{$item->descripcion_small}} </p></dd>
                                        </dl>
                                        @break
                                    @default
                            ...
                            @endphp
                            <td>
                                <h5 class="text-dark" id="label-subtotal{{$item->id}}">Bs. {{number_format($precio_actual + $envio, 2, ',', '')}}</h5>
                                <input type="hidden" class="input-subtotal" id="input-subtotal{{$item->id}}" value="{{$precio_actual + $envio}}">
                            </td>
                            <td class="text-right">
...
@endsection
<script>
    const cont_carrito = parseInt('{{$cont}}');
    function calcular_total(id){
        let cantidad = $('#input-cantidad'+id).val();
        $.get("{{url('carrito/get_precio')}}/"+id+'/'+cantidad, function(data){
        ...
        $(document).ready(function(){
            ...
            $('#pills-tab2-tab').click(function(){
                ...
                setTimeout(()=>{
                    ...
                    map2 = L.map('map2').setView([{{$sucursal->latitud}}, {{$sucursal->longitud}}], 15);
                    var iconoBase = L.Icon.extend({ options: { iconSize: [40, 40], iconAnchor: [15, 35], popupAnchor: [0, -30] } });
                    let iconDelivery = new iconoBase({iconUrl: "{{url('storage').'/'.str_replace('\\', '/', setting('empresa.logo'))}}"});
                    ...
                    L.marker([{{$sucursal->latitud}}, {{$sucursal->longitud}}], {icon: iconDelivery}).addTo(map2)
