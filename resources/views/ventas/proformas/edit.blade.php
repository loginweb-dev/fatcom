        ...
        <script>
            ...
            function seleccionar_producto(){
                ...
                $.get("{{url('admin/productos/get_producto')}}/"+id, function(data){
                    let stock = data.se_almacena ? data.stock : 1000;
                    agregar_detalle_venta(data.id, data.nombre, data.precio,stock, data.unidades);
            ...
            function agregar_detalle_venta(id, nombre, precio,stock,unidades,cantidad=0){
                if(stock<1){
                ...
                if(existe){
                    ...
                    $('#detalle_venta').before(`<tr class="tr-detalle" id="tr-${index_tr}" data-id="${index_tr}">
                                                    ...
                                                    <td>
                                                        <div class="input-group">
                                                            <input type="number" id="input-precio_${index_tr}" min="1" step="0.01" value="${precio}" name="precio[]" class="form-control" onchange="subtotal('${index_tr}');" onkeyup="subtotal('${index_tr}');" required />
                                                            <span class="input-group-addon">Bs.</span>
                                                        </div>
                                                    </td>
                                                    <td><input type="number" min="0.1" max="${stock}" step="0.01" class="form-control" id="input-cantidad_${index_tr}" value="${cantidad}" name="cantidad[]" onchange="subtotal('${index_tr}', '${id}')" onkeyup="subtotal('${index_tr}', '${id}')" required></td>
                                                    <td class="label-subtotal" id="subtotal-${index_tr}"><h4>${precio} Bs.</h4></td>
                                                    <td width="40px"><label onclick="borrarProducto('${index_tr}')" class="text-danger" style="cursor:pointer;font-size:20px"><span class="voyager-trash"></span></label></td>
                                                <tr>`);
